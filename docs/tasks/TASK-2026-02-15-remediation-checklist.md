...
| R3 | 条款抽取与建模不完整 | Partial | requirement 增加结构化字段（章节路径、约束类型、数值条件、对象） | 至少支持“数量/金额/期限”3类约束抽取；抽取准确率在基准集达到目标阈值 |
| R4 | 缺少“硬性否决项/评分项”分层判定 | Done | requirement 增加 `rule_tier`（hard_fail/scored/general） | 审查输出分层统计；hard_fail 召回率单独统计；manual checklist 按层级排序 |
...
| R8 | 缺少稳定评测基准，质量不可量化 | Partial | 建立固定评测集与指标板（漏检率/误报率/hard_fail召回） | 新增评测命令可重复执行；PR 必须附核心指标变化 |
